...
{
  double ptx[ECLIPSEPOINTS], pty[ECLIPSEPOINTS]; // 頂点座標位置
  ...
  for(i=0; i<ECLIPSEPOINTS; i++) {
    th = 2*M_PI * ((double)i/ECLIPSEPOINTS);
    xx=cos(th)*w;
    yy=sin(th)*h;
  ...
  /* 描画 */
  fillpoly(win, ptx, pty, ECLIPSEPOINTS, FLAG);
}
#undef ECLIPSEPOINTS
...
#define POLYNUM 90 // 90 角形として描く
    {
      double posx[POLYNUM], posy[POLYNUM]; // 頂点位置座標配列
...
#undef POLYNUM
...

    {
      ...
      newpen(win, 2);  // 赤色に設定
      fillpoly(win, posx, posy, 4, FLAG); // 長方形を描く
      ...
      newpen(win, 3);  // 緑色に設定
      fillpoly(win, newx, newy, 4, FLAG); // 回転後の長方形を描く
    }
...
#define LENGTH 150.0
    {
      ...
      fillpoly(win, newx, newy, POLYNUM, FLAG); // 多角形を描く
    }
#undef LENGTH
#undef POLYNUM
