 ...
 * TrueType fontsの設定
 * 引数　int wn: ウィンドウ番号
 *      char *fontsfile:   TrueType fontsのファイル名
 * 戻り値　なし
 */
static void lowsetfonts(int wn,const char fontsfile[])
{
	int error;
	
	if(Pc[wn].fontload != 0) { //過去に読み込まれたフォントデータをクリア
		error = FT_Done_Face( Pc[wn].face );
		if(error) {
			fprintf(stderr,"error FT_Done_Face\n");
			exit(EXIT_FAILURE);
		}
		error = FT_Done_FreeType( Pc[wn].library );
		if(error) {
    ...
    error = FT_Init_FreeType( &Pc[wn].library );
    if(error) {
		fprintf(stderr,"error FT_Init_FreeType\n");
		exit(EXIT_FAILURE);
    }
    error = FT_New_Face( Pc[wn].library, fontsfile, 0, &Pc[wn].face );
    if(error == FT_Err_Unknown_File_Format) {
		fprintf(stderr,"unsupport font format\n");
		exit(EXIT_FAILURE);
    } else if(error) {
		fprintf(stderr,"font file not found\n");
		exit(EXIT_FAILURE);
    }
    Pc[wn].slot = Pc[wn].face->glyph;
    Pc[wn].fontload = 1;
}
 ...
 * 戻り値　なし
 */
static void lowdrawchar(int wn,int draw_x,int draw_y,double angle,int fontssize,char text[])
{
    int error;
    wchar_t *ws; /* ワイド文字保存用 */
    ...
    error = FT_Set_Pixel_Sizes( Pc[wn].face, 0, fontssize ); //ピクセル単位でサイズ指定
	...
	for (int n = 0;n < num_chars; n++ ){
	    FT_Bitmap bitmap;
	    FT_Set_Transform( Pc[wn].face, &matrix, &pen );
	    switch(ws[n]) {
		case '\r':
		    pen.x  =  draw_x*64 + cos(angle-M_PI/2) * line * (Pc[wn].face->size->metrics.height);
		    pen.y  = -draw_y*64 + sin(angle-M_PI/2) * line * (Pc[wn].face->size->metrics.height);
		    break;
		case '\t': /*tabは正しく計算していません */
		    pen.x  = pen.x - sin(angle-M_PI/2)*(Pc[wn].face->size->metrics.max_advance);
		    pen.y  = pen.y + cos(angle-M_PI/2)*(Pc[wn].face->size->metrics.max_advance);
		    break;
		case '\n':
		    line++;
		    pen.x  =  draw_x*64 + cos(angle-M_PI/2) * line * (Pc[wn].face->size->metrics.height);
		...
		default:
		    // n文字目の文字をビットマップ化
		    error = FT_Load_Char( Pc[wn].face, ws[n], FT_LOAD_RENDER |FT_LOAD_MONOCHROME );// 横書き アンチエリアシングなし
		    if(error) {
				fprintf(stderr,"FT_Load_Char error\n");
				exit(EXIT_FAILURE);
		    }
		    bitmap = Pc[wn].slot->bitmap;
		    ...
		    for( int loopy = 0; loopy < bitmap.rows ; loopy++){
				for(int byte_index=0;byte_index<bitmap.pitch;byte_index++) {
					...
					int bit = byte_value & (1<<(7-bit_index));
						if(bit!=0) {
							int x = (bit_index+rowstart)%bitmap.width  + Pc[wn].slot->bitmap_left;
							int y = (bit_index+rowstart)/bitmap.width - Pc[wn].slot->bitmap_top;
							lowpset(wn,x,y); /* 点を描く */
						}
					}
				}
		    }
		    pen.x += Pc[wn].slot->advance.x ;
		    pen.y += Pc[wn].slot->advance.y ;
		  ...
		  const char *argsformat, ... )
{
    ...
    xyconv(wn,x,y,&xxg,&yyg) ;
    theta = theta * M_PI / 180.0; //ラジアンに変換

    len=strlen(str) ;
    lowdrawchar(wn,xxg,yyg,theta,size,str);
