...
int main()
{
    ...
    win = gopen(800,600) ;
    layer(win,0,1) ;
	//newfontset(win,"/usr/share/fonts/opentype/noto/NotoSansCJK-Bold.ttc") ;
    //if( i ){
    ...
    for( i=0 ; i < len/3 ; i++ ){
		...
		gclr(win) ;
		drawstr(win,100+20,600/2-560/2+560/6,600,0,one) ;
		copylayer(win,1,0) ;
		msleep(220) ;
    }
    for( i=0 ; i<8 ; i++ ){
		...
		//		fsize) ;
		drawstr(win,800/2-(len/3)*fsize/2,600/2-fsize/2+fsize/6,fsize,0,rupin) ;
		copylayer(win,1,0) ;
		msleep(20) ;
    }
    sleep(2) ;
