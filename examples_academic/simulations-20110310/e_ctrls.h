                  ...
                  int isme, int type, int button)
{
  ...
  for (ri = 0; ri < num; ri++) {
    ...
    if (c[ri].name[0] != '_') {
      ...
      gputimage(w, 4, HTSTR*i,Xpm_image_flat, 130, 19, 1);
      gputimage(w, 136, HTSTR*i,Xpm_image_flat, 130, 19, 1);
      newcolor(w, ECTRL_FGCOLOR);
      drawstr(w, 8, HTSTR*i+4, FONTSET, 0, c[ri].name);
      drawstr(w, 142, HTSTR*i+4, FONTSET, 0, "%.6g", *c[ri].rval);
      gputimage(w, 270, HTSTR*i+1,Xpm_image_l, 14, 16, 1);
      gputimage(w, 285, HTSTR*i+1, Xpm_image_r,14, 16, 1);
      ...
      gputimage(w, 269, HTSTR*i+2,sw[(int)(*c[ri].rval)], 31,18, 1);
    }
  }
  copylayer(w, 1, 0);
  return 1;
